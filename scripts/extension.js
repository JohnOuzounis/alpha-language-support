...
function activate(context) {
    ...
    function getBinPath() {
        if (process.platform === 'win32') return 'bin\\windows';
        else if (process.platform === 'linux') return 'bin/linux';
        else throw Error('Unsupported platform');
    }

    function getParserPath() {
        if (process.platform === 'win32') return 'parser.exe';
        else if (process.platform === 'linux') return 'parser';
        else throw Error('Unsupported platform');
    }

    function getCompilerPath() {
        if (process.platform === 'win32') return 'alphac.exe';
        else if (process.platform === 'linux') return 'alphac';
        else throw Error('Unsupported platform');
    }

    function getVmPath() {
        if (process.platform === 'win32') return 'alpha.exe';
        else if (process.platform === 'linux') return 'alpha';
        else throw Error('Unsupported platform');
    }
    ...
    function parse() {
        ...
        if (activeEditor && activeEditor.document.languageId === 'alpha') {
            ...
            const binFolderUri = vscode.Uri.joinPath(
                context.extensionUri,
                getBinPath(),
            );
            const parserPath = vscode.Uri.joinPath(
                binFolderUri,
                getParserPath(),
            ).fsPath;
    ...
    function compileAndRunVM(args) {
        ...
        if (activeEditor) {
            const activeFilePath = activeEditor.document.uri.fsPath;
            const activeFileName = path.join(
            ...
            const compilerPath = vscode.Uri.joinPath(
                binFolderUri,
                getCompilerPath(),
            ).fsPath;
            const vmPath = vscode.Uri.joinPath(
                binFolderUri,
                getVmPath(),
            ).fsPath;
    ...
    let disposableParseCommand = vscode.commands.registerCommand(
        'extension.parseGrammar',
        () => {
            parse();
        },
    );

    let disposableRunCommand = vscode.commands.registerCommand(
        'extension.compileAndRunVM',
        args => {
